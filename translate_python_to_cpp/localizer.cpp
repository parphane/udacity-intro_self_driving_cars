...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	float initial_prob = 1.0 / (grid.size() * grid[0].size());
	
	return vector< vector <float> > (grid.size(), vector <float>(grid[0].size(), initial_prob));
}
  ...
  float blurring) 
{
    ...
    for (int i=0; i<height; i++) {
		for (int j=0; j<width; j++) {
            ...
            new_beliefs[int(new_i)][int(new_j)] = beliefs[i][j];
		}
	}

  return blur(new_beliefs, blurring);
}
