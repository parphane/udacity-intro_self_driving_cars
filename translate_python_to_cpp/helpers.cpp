...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	vector< vector<float> > new_grid(grid.size(), vector <float>(grid[0].size(), 0));

	// Got some suggestion from a good friend of mine on accumulators
	float total = accumulate( 
		begin(grid)
		, end(grid)
		, 0.f
		, [](float a, vector<float> b) {
			return accumulate( begin(b), end(b), a);
		}
	);
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	vector< vector <float> > new_grid(height, vector <float>(width, 0));
...
/* Extracted from https://stackoverflow.com/a/48419381 */
int mod(int a, int b) {
	int ret = a%b;
